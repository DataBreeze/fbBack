...
class FileReplyTable extends Doctrine_Table{

  const REPLY_RECORD_LIMIT = 10;

  public static function getInstance(){
    return Doctrine_Core::getTable('FileReply');
  }
  ...
  public function getReplies($pid,$limit = FileReplyTable::REPLY_RECORD_LIMIT,$offset = 0){
    ...
    $q = Doctrine_Query::create()
      ...
      ->limit($limit);
    if($offset > 0){
      $q->offset($offset * FileReplyTable::REPLY_RECORD_LIMIT);
    }
    $rows = $q->execute(array(),Doctrine_Core::HYDRATE_SCALAR);
    $recs = array();
