...
class SpotReplyTable extends Doctrine_Table{

  const REPLY_RECORD_LIMIT = 20;

  public static function getInstance(){
    return Doctrine_Core::getTable('SpotReply');
  }
  ...
  public function getReplies($pid,$limit = SpotReplyTable::REPLY_RECORD_LIMIT,$offset = 0){
    ...
    $q = Doctrine_Query::create()
      ...
      ->limit($limit);
    if($offset > 0){
      $q->offset($offset * SpotReplyTable::REPLY_RECORD_LIMIT);
    }
    $rows = $q->execute(array(),Doctrine_Core::HYDRATE_SCALAR);
    $recs = array();
